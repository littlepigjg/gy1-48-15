...
import { PoisonGasCloud, RadiationField, HazardManager } from '../src/game/hazards.js';
...
describe('HazardManager Radiation', () => {
  let hm;
  let mockPlayer;
  let mockWorld;
  let damageEvents;
  ...
  it('初始radiationFields应该为空', () => {
    expect(hm.radiationFields.length).toBe(0);
  });

  it('spawnRadiationField创建辐射场', () => {
    hm.spawnRadiationField(400, 600);
    expect(hm.radiationFields.length).toBe(1);
    expect(hm.radiationFields[0] instanceof RadiationField).toBe(true);
  });

  it('玩家在辐射场中会受到辐射伤害', () => {
    hm.spawnRadiationField(mockPlayer.x, mockPlayer.y);
    const startHealth = mockPlayer.health;
  ...
  it('敌人在辐射场中会受到伤害并可能变异', () => {
    ...
    for (let i = 0; i < Math.ceil(RADIATION.TICK_INTERVAL / 0.1) + 2; i++) {
      hm.update(0.1, mockWorld, mockPlayer, () => {}, mockEnemies);
    }
  ...
  it('update会清理过期的辐射场', () => {
    hm.spawnRadiationField(400, 600);
    expect(hm.radiationFields.length).toBe(1);
    hm.radiationFields[0].life = 0.1;

    hm.update(1, mockWorld, mockPlayer, () => {}, []);
